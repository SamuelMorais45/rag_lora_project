...
def _tokenize_causal(samples: list[dict], tokenizer, max_len: int = 512) -> Dataset: 
    texts = [
        build_prompt(s["Instruction"], s["Output"]) + tokenizer.eos_token
        for s in samples
    ]
    enc = tokenizer(
        texts,
...
def _tokenize_seq2seq(samples: list[dict], tokenizer,
                      max_src: int = 256, max_tgt: int = 256) -> Dataset:
    ...
    enc["labels"] = [
        [(t if t != pad_id else -100) for t in row]
...
def _plot_loss(history: list[float], save_dir: str, name: str):
    if not history:
        return
    ...
    plt.close()
    print(f"[INFO] Curva de loss salva: {path}")
