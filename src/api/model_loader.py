...
def _load_causal(model_id: str, lora_path: str) -> dict | None:
...
def _load_seq2seq(model_id: str, lora_path: str) -> dict | None:
    if not Path(lora_path).exists():
        logger.warning("[LOADER] Caminho não encontrado: %s — pulando.", lora_path)
        return None
    ...
    return {"pipeline": None, "tipo": "seq2seq", "tokenizer": tokenizer, "model": model}


def load_all_models() -> dict:
    models = {}

    models["causal-gpt-neo-125m"] = _load_causal(   
    ...
    models["seq2seq-mt5-small"] = _load_seq2seq(    
